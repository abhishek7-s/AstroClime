...
// We are explicitly casting the prototype to 'any'.
// This is a direct command to the TypeScript compiler to suspend all type-checking
// for this one specific operation. It's a brute-force solution that overrides the
// strict build-time error.
...
// Helper component to automatically fly the map to a new position
function MapFlyToController({ position }: { position: PositionState }) {
  const map = useMap();
  useEffect(() => {
    map.flyTo(position, map.getZoom());
